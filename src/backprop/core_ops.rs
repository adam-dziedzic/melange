...
use crate::tensor::allocation_policy::{StaticAllocationPolicy, DynamicAllocationPolicy};
use crate::tensor::prelude::*;
use crate::tensor::transpose_policy::Contiguous;
use melange_macros::{define_closure, expand_impl};
use std::cell::RefCell;
use std::rc::Rc;

#[expand_impl(
    add<T: Add<Output = T> + 'static, D=Static, Drhs=Static,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        Srhs=S
    > in Add,
    add<T: Add<Output = T> + 'static, D=Static, Drhs=Dynamic,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        S: Same<Srhs>,
    ...
    > in Add,
    add<T: Add<Output = T> + 'static, D=Dynamic, Drhs=Static,
        Sout=Srhs,
        Sback=Srhs,
        Sbackrhs=Srhs,
        S: Same<Srhs>,
        <S as Same<Srhs>>::Output: TRUE,
    > in Add,
    add<T: Add<Output = T> + 'static, D=Dynamic, Drhs=Dynamic,
        Sout=<S as ReprShape<T, Srhs>>::Output,
        Sback=<S as ReprShape<T, Srhs>>::Output,
        Sbackrhs=<S as ReprShape<T, Srhs>>::Output,
        S: Same<Srhs> + ReprShape<T, Srhs>,
        <S as Same<Srhs>>::Output: TRUE,
        S: Same<<S as ReprShape<T, Srhs>>::Output> + ReprShape<T, Srhs>,
        <S as Same<<S as ReprShape<T, Srhs>>::Output>>::Output: TRUE,
        Srhs: Same<<S as ReprShape<T, Srhs>>::Output> + ReprShape<T, Srhs>,
        <Srhs as Same<<S as ReprShape<T, Srhs>>::Output>>::Output: TRUE,
    > in Add,
    sub<T: Sub<Output = T> + Ring + Neg<Output = T> + Mul<Output = T> + 'static, D=Static, Drhs=Static,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        Srhs=S
    > in Sub,
    sub<T: Sub<Output = T> + Ring + Neg<Output = T> + Mul<Output = T> + 'static, D=Static, Drhs=Dynamic,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        S: Same<Srhs>,
        ...
        <Srhs as Same<S>>::Output: TRUE,
    > in Sub,
    sub<T: Sub<Output = T> + Ring + Neg<Output = T> + Mul<Output = T> + 'static, D=Dynamic, Drhs=Static,
        Sout=Srhs,
        Sback=Srhs,
        Sbackrhs=Srhs,
        S: Same<Srhs>,
        <S as Same<Srhs>>::Output: TRUE,
    > in Sub,
    sub<T: Sub<Output = T> + Ring + Neg<Output = T> + Mul<Output = T> + 'static, D=Dynamic, Drhs=Dynamic,
        Sout=<S as ReprShape<T, Srhs>>::Output,
        Sback=<S as ReprShape<T, Srhs>>::Output,
        Sbackrhs=<S as ReprShape<T, Srhs>>::Output,
        S: Same<Srhs> + ReprShape<T, Srhs>,
        ...
        <Srhs as Same<<S as ReprShape<T, Srhs>>::Output>>::Output: TRUE,
    > in Sub,
    mul<T: MulAssign + Mul<Output = T> + 'static, D=Static, Drhs=Static,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        Srhs=S
    > in Mul,
    mul<T: MulAssign + Mul<Output = T> + 'static, D=Static, Drhs=Dynamic,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        S: Same<Srhs>,
        ...
        <Srhs as Same<S>>::Output: TRUE,
    > in Mul,
    mul<T: MulAssign + Mul<Output = T> + 'static, D=Dynamic, Drhs=Static,
        Sout=Srhs,
        Sback=Srhs,
        Sbackrhs=Srhs,
        S: Same<Srhs>,
        ...
        <Srhs as Same<S>>::Output: TRUE,
    > in Mul,
    mul<T: MulAssign + Mul<Output = T> + 'static, D=Dynamic, Drhs=Dynamic,
        Sout=<S as ReprShape<T, Srhs>>::Output,
        Sback=<S as ReprShape<T, Srhs>>::Output,
        Sbackrhs=<S as ReprShape<T, Srhs>>::Output,
        S: Same<Srhs> + ReprShape<T, Srhs>,
        ...
        Srhs: Same<<S as ReprShape<T, Srhs>>::Output> + ReprShape<T, Srhs>,
        <Srhs as Same<<S as ReprShape<T, Srhs>>::Output>>::Output: TRUE,
        <S as ReprShape<T, Srhs>>::Output: Same<S>,
        <<S as ReprShape<T, Srhs>>::Output as Same<S>>::Output: TRUE,
        <S as ReprShape<T, Srhs>>::Output: Same<Srhs>,
        <<S as ReprShape<T, Srhs>>::Output as Same<Srhs>>::Output: TRUE,
    > in Mul,
    div<T: DivAssign + Div<Output = T> + Mul<Output = T> + 'static, D=Static, Drhs=Static,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        Srhs=S
    > in Div,
    div<T: DivAssign + Div<Output = T> + Mul<Output = T> + 'static, D=Static, Drhs=Dynamic,
        Sout=S,
        Sback=S,
        Sbackrhs=S,
        S: Same<Srhs>,
        ...
        <Srhs as Same<S>>::Output: TRUE,
    > in Div,
    div<T: DivAssign + Div<Output = T> + Mul<Output = T> + 'static, D=Dynamic, Drhs=Static,
        Sout=Srhs,
        Sback=Srhs,
        Sbackrhs=Srhs,
        S: Same<Srhs>,
        ...
        <Srhs as Same<S>>::Output: TRUE,
    > in Div,
    div<T: DivAssign + Div<Output = T> + Mul<Output = T> + 'static, D=Dynamic, Drhs=Dynamic,
        Sout=<S as ReprShape<T, Srhs>>::Output,
        Sback=<S as ReprShape<T, Srhs>>::Output,
        Sbackrhs=<S as ReprShape<T, Srhs>>::Output,
        S: Same<Srhs> + ReprShape<T, Srhs>,
...
impl<T, S, D, C, L, P, Cback, Lback, Pback, Srhs, Drhs, Crhs, Lrhs, Prhs, Lgrad, Lgradrhs>
    Operation<Variable<T, Srhs, Drhs, Crhs, Lrhs, Prhs, Sbackrhs, Static, Contiguous, Pback::Layout, Pback, Lgradrhs>> for Variable<T, S, D, C, L, P, Sback, Static, Cback, Lback, Pback, Lgrad>
where
    ...
    // Gradient accumulation in Output variable.
    P::Layout: for<'a> LayoutMut<'a, T> + for<'a> Layout<'a, T>,
    Lback: for<'a> Layout<'a, T>,

    // Gradient accumulation in Lhs variable.
    L: for<'a> LayoutMut<'a, T>,
    Lgrad: for<'a> LayoutMut<'a, T> + for<'a> Layout<'a, T>,
    Lback: for<'a> Layout<'a, T>,

    // Gradient accumulation in Rhs variable.
    Lrhs: for<'a> LayoutMut<'a, T>,
    Lgradrhs: for<'a> LayoutMut<'a, T> + for<'a> Layout<'a, T>,
    Pback::Layout: for<'a> Layout<'a, T>,
    
    // Rhs-bound gradient allocation in Output variable closure.
    Pback: StaticAllocationPolicy<T, Sbackrhs>,

    // In place ops on Lhs-bound gradient in Output variable closure.
    Lback: for<'a> LayoutMut<'a, T>,

    // Closure static bounds
    S: 'static,
    C: 'static,
    L: 'static,
    P: 'static,
    ...
    Lgradrhs: 'static,
{
    type Output = Variable<T, Sout, Static, Contiguous, P::Layout, P, Sout, Static, Cback, Lback, Pback, P::Layout>;
    
    fn operation(self, other: Variable<T, Srhs, Drhs, Crhs, Lrhs, Prhs, Sbackrhs, Static, Contiguous, Pback::Layout, Pback, Lgradrhs>) -> Self::Output {
        let (value, grad) = {
            let self_ref = self.borrow();
            let other_ref = other.borrow();
